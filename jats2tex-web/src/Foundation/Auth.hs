...
data ForgotPasswordForm = ForgotPasswordForm
  { _forgotEmail :: Text
  }

data PasswordForm = PasswordForm
  { _passwordCurrent :: Text
  , _passwordNew     :: Text
  ...
  where
    loginForm extra = do
      ...
      (passwordRes, passwordView) <-
        mreq passwordField (passwordSettings passwordMsg) Nothing
      let userRes =
            UserLoginForm Control.Applicative.<$> emailRes Control.Applicative.<*>
            passwordRes
      let widget =
            [whamlet|
                #{extra}
...
bootstrapSetPasswordHandler
  :: YesodAuthEmail master
  => Bool -> HandlerT Auth (HandlerT master IO) TypedContent
bootstrapSetPasswordHandler needOld = do
  ...
  where
    setPasswordForm extra = do
      (currentPasswordRes, currentPasswordView) <-
        mreq passwordField currentPasswordSettings Nothing
      (newPasswordRes, newPasswordView) <-
        mreq passwordField newPasswordSettings Nothing
      (confirmPasswordRes, confirmPasswordView) <-
        mreq passwordField confirmPasswordSettings Nothing
      let passwordFormRes =
            PasswordForm <$> currentPasswordRes <*> newPasswordRes <*>
            confirmPasswordRes
      let widget =
            [whamlet|
                #{extra}
...
bootstrapForgotPasswordHandler
  :: YesodAuthEmail master
  => HandlerT Auth (HandlerT master IO) Html
