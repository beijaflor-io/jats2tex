...
module Text.JaTex.TexWriter where
...
runTexWriter ::
     Monad m => TexState -> StateT TexState m t -> m (TexState, LaTeX, t)
...
convert fp tmp i w = do
  ...
  (_, !t, _) <-
    runTexWriter
      emptyState
      {tsFileName = fp, tsTemplate = tmp, tsDebug = debug, tsWarnings = w}
      (jatsXmlToLaTeX i)
  return t

jatsXmlToLaTeX :: MonadTex m => JATSDoc -> m ()
jatsXmlToLaTeX d = do
  add $
    comment
      (Text.pack
...
convertNode :: MonadTex m => HXT.XmlTree -> m (LaTeXT Identity ())
...
convertElem :: MonadTex m => HXT.XmlTree -> m (LaTeXT Identity ())
convertElem el@(HXT.NTree (HXT.XTag name attrs) children) = do
  ...
  where
    ...
    getTemplateContext = do
      ...
      return TemplateContext {tcLuaState = l, tcState = st, tcElement = el}
    run =
      case children of
        ...
        _ -> do
          ...
          convertChildren el
convertElem e =
  fail $ "convertElem needs XML elements but got (" <> show e <> ")"
...
convertInlineNode ::
     MonadTex m => HXT.XmlTree -> m ([LaTeXT Identity ()], [LaTeXT Identity ()])
...
convertInlineChildren el = do
  ...
  (newState, _, _) <-
    runTexWriter
      (st {tsHeadRev = mempty, tsBodyRev = mempty})
      (convertChildren el)
  return (tsHead newState, tsBody newState)
...
begin :: Monad m => Text -> LaTeXT m () -> LaTeXT m ()
begin n c =
...
evalNode e ptn (heads, bodies) = do
  ...
  case ptn of
    ...
    (PreparedTemplateVar "heads") ->
      return $ render . runLaTeX . sequence_ $ heads
    (PreparedTemplateVar "bodies") ->
      return $ render . runLaTeX . sequence_ $ bodies
    (PreparedTemplateVar "children") ->
      return $ render . runLaTeX . sequence_ $ children
    (PreparedTemplateVar "requirements") ->
      return $ render (runLaTeX requirements)
    (PreparedTemplateVar _) -> return ""
    (PreparedTemplateLua run) -> do
      (_, _, result) <-
        liftIO $ runTexWriter (tcState e) (run e (heads, bodies))
  ...
  where
    mkFindChildren ::
         MonadTex m => TemplateContext -> Text -> m (LaTeXT Identity ())
...
prepareInterp i =
  ...
  where
    doPrepare ::
         TemplateInterpNode -> IO (PreparedTemplateNode (StateT TexState IO))
          ...
          where
            ...
            luaChildren
             ...
             = do
              return $
                Text.encodeUtf8 $
                render . runLaTeX . sequence_ $ (heads <> bodies)
...
parseTemplateNode ::
     ConcreteTemplateNode
  -> IO (Either Text (TemplateNode (StateT TexState IO)))
...
parseCTemplateFromJson _ =
  Left ["Template inválido, o formato esperado é `seletor: 'template'`"]
